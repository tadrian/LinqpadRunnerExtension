...
- GitHub links corrected in package.json for marketplace### ✨ Complete Release - Interactive Results Viewer + Navigation### 🎨 Major Feature - Rich Visualizations

- Cleaned up old v1.9 preview features (charts, images, HTML rendering) - not ready for release

- Focus back on stable core features: Interactive Viewer, F12 Navigation, Output Control#### Interactive Results Viewer- **📊 Interactive Charts**: Render charts using Chart.js (line, bar, pie, doughnut, multi-dataset)
...
### ✨ Complete Release - Interactive Results Viewer + Navigation
...
- **WebView-based Side-by-Side Panel** - View results without leaving VS Code- **CSV Export** - Export table data as CSV files with one click- **VS Code Theming**: Charts adapt to VS Code's color theme

- **JSON Tree Visualization** - Expandable/collapsible objects and arrays with syntax highlighting

- **Sortable Tables** - Click column headers to sort, search box to filter rows- **Collapsible Sections** - Organize multiple outputs in expandable sections

- **CSV Export** - Export table data as CSV files with one click

- **Collapsible Sections** - Organize multiple outputs in expandable sections- **Live Output Updates** - Results update as script executes#### Chart Data Format

- **Live Output Updates** - Results update as script executes
...
- `linqpadRunner.outputDestination` setting:
  ...
  - `"both"` - Display in console and viewer (default)#### Output Destination Control{
...
- **One-Click Execution** with play button#### F12 Navigation Features```
...
- Fixed auto-injection bug with comments containing "Dumpify"
